...
def main():
    ...
    connection = psycopg2.connect(
        host="127.0.0.1", database="imagedb", user="postgres", password=db_password
    )
...
def insert_into_label_names(bucket, connection, cursor):
    ...
    for line in lines:
        item = line.split(",")
        cursor.execute(
            "INSERT INTO label_names (label, name) VALUES (%s, %s)", (item[0], item[1])
        )
    connection.commit()
...
def insert_into_image_labels(bucket, connection, cursor):
    ...
    for line in lines:
        if line_count > 0:
            item = line.split(",")
            cursor.execute(
                "INSERT INTO image_labels (imageid, source, label, confidence) VALUES (%s, %s, %s, %s)",
...
def insert_into_image_bbox(bucket, connection, cursor):
    ...
    for line in lines:
        if line_count > 0:
            item = line.split(",")
            cursor.execute(
                "INSERT INTO image_bbox (imageid, source, label, confidence, x_min, x_max, y_min, y_max, is_occ, is_tru, is_grp, is_dep, is_ins) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                (
                    item[0],
                    item[1],
                    item[2],
    ...
    for line in lines:
        if line_count > 0:
            ...
            if imageid != last_imageid:
                cursor.execute(
                    "INSERT INTO image_bboxes (imageid, label, x_min, x_max, y_min, y_max, fhb_x_min, fhb_x_max, fhb_y_min, fhb_y_max, fvb_x_min, fvb_x_max, fvb_y_min, fvb_y_max, rb_x_min, rb_x_max, rb_y_min, rb_y_max) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (
                        imageid,
                        labels,
                        x_mins,
...
def insert_into_image_selection(bucket, connection, cursor):
    ...
    for line in lines:
        item = line.split(",")
        cursor.execute(
            "INSERT INTO image_selection (imageid, obj_count, is_google, label) VALUES (%s, %s, %s, %s)",
