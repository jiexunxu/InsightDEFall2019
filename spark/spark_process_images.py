...
def transform(internal_params, s3_image_files, user_param):
    ...
    flip_horizontal_blur_trans = (
        ImageTransformer()
        .setOutputCol("fhb_")
        .resize(L, L)
        .flip(flipCode=1)
        .gaussianKernel(window, sigma)
    )
    # Resize image to L, flip vertically, and apply gaussian blur
    flip_vertical_blur_trans = (
        ImageTransformer()
        .setOutputCol("fvb_")
        .resize(L, L)
        .flip(flipCode=0)
    ...
    scale_blur_trans = (
        ImageTransformer()
        .setOutputCol("sb_")
        .resize(int(L * sc), int(L * sc))
        .crop(int(L * (sc - 1) / 2), int(L * (sc - 1) / 2), L, L)
        .gaussianKernel(window, sigma)
    )
    ...
    crop_blur_trans = (
        ...
        .setOutputCol("cb_")
        .resize(L, L)
        .crop(int(L * cx1), int(L * cy1), int(L * (cy2 - cy1)), int(L * (cx2 - cx1)))
        .resize(L, L)
        .gaussianKernel(window, sigma)
    )
