...
def save(connection, query, user_param, output_foldername):
    ...
    header = "ImageID,label,x_min,x_max,y_min,y_max,fhb_x_min,fhb_x_max,fhb_y_min,"
    header += "fhb_y_max,fvb_x_min,fvb_x_max,fvb_y_min,fvb_y_max,rb_x_min,rb_x_max,"
    header += "rb_y_min,rb_y_max,sb_x_min,sb_x_max,sb_y_min,sb_y_max,cb_x_min,cb_x_max,cb_y_min,cb_y_max\n"
    with s3.open(
        "jiexunxu-open-image-dataset/output_data/"
        + output_foldername
        ...
        for k in range(len(bbox_descriptors)):
            ...
            for i in range(len(row[1])):
                ...
                f.write(
                    ...
                    + str(1 - y_max)
                    + ","
                    + str((x_min - (sc - 1) / 2) / (2 - sc))
                    + ","
                    + str((x_max - (sc - 1) / 2) / (2 - sc))
                    + ","
                    + str((y_min - (sc - 1) / 2) / (2 - sc))
                    + ","
                    + str((y_max - (sc - 1) / 2) / (2 - sc))
                    + ","
                    + str((x_min - cx1) / (cx2 - cx1))
                    + ","
                    + str((x_max - cx1) / (cx2 - cx1))
                    + ","
                    + str((y_min - cy1) / (cy2 - cy1))
                    + ","
                    + str((y_max - cy1) / (cy2 - cy1))
                    + "\n"
                )
