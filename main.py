...
def start():
    ...
    all_bbox_descriptor=[]
    local_file_name1="tmp1.txt"
    local_file1=open(local_file_name1, 'w')
    local_file1.write("ImageID,Source,LabelName,Confidence\n")
    local_file1.close()
    local_file1.open(local_file_name1, 'a')
    local_file_name2="tmp2.txt"
    local_file2=open(local_file_name2, 'w')
    local_file2.write("ImageID,Source,LabelName,Confidence,XMin,XMax,YMin,YMax,IsOccluded,IsTruncated,IsGroupOf,IsDepiction,IsInside\n")
    local_file2.close()
    ...
    for imageid in imageids:
        ...
        save_metadata.save(bucket, connection, imageid, bbox_descriptor, bbox_xy_enhanced, output_foldername, local_file1, local_file2)
        image_count+=1
        print('enhanced {} images'.format(image_count))
    local_file1.close()
    local_file2.close()
    with open(local_file1, 'rb') as body:
        bucket.put_object(Key=output_foldername+'selected-train-annotations-human-imagelabels-boxable.csv', Body=body)
    with open(local_file2, 'rb') as body:
