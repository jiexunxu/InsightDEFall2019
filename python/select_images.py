...
def select(connection, max_obj_count, is_google, target_label):
    ...
    if is_google==1:
        cursor.execute('''SELECT imageid FROM image_selection WHERE obj_count<=%s AND is_google=1 AND label=%s''', (max_obj_count, target_label))
    else:
        cursor.execute('''SELECT imageid FROM image_selection WHERE obj_count<=%s AND label=%s''', (max_obj_count, target_label))
