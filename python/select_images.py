...
def select(connection, user_selection):
    ...
    if is_google == 1:
        cursor.execute(
            """SELECT imageid FROM image_selection WHERE obj_count>=%s AND obj_count<=%s AND is_google='1' AND label IN %s""",
            (min_obj_count, max_obj_count, target_labels),
        )
    else:
        cursor.execute(
            """SELECT imageid FROM image_selection WHERE obj_count>=%s AND obj_count<=%s AND label IN %s""",
            (min_obj_count, max_obj_count, target_labels),
        )
